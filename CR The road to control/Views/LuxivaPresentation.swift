...
struct TemoraRootView: View {
    ...
    private func checkDateAndProceed() {
        ...
        print("🔬 Проверка даты: Пройдена - продолжаем стандартный путь")
        
        // ШАГ 2: Если дата прошла, проверяем первый ли это вход
        if CoachModExerciseService.instance.traineeModIsFirstLaunch() {
            // Первый вход - запускаем ATT запрос (только после проверки даты!)
            print("🔬 Запускаем ATT запрос (дата прошла, первый вход)")
            ...
            // Ждем ATT перед проверками веб вью
    ...
    private func continueWithWebViewChecks() {
        // После ATT выполняем проверки веб вью
