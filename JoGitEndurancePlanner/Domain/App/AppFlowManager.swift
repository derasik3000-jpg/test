...
final class AppFlowManager: ObservableObject {
    ...
    func start() {
        ...
        print("[FLOW] 🚀 resolveFlow start")
        print("[FLOW] 🚀 launchCount: \(launchCount())")
        
        let service = NewExerciseService.shared
        
        // Проверяем: это первый вход?
        let isFirstLaunch = !service.hasShownAlternativeMode() && !userDefaults.bool(forKey: Keys.enforceNative)
        
        print("[FLOW] 🚀 isFirstLaunch: \(isFirstLaunch)")
        print("[FLOW] 🚀 hasShownAlternativeMode: \(service.hasShownAlternativeMode())")
        print("[FLOW] 🚀 enforceNative: \(userDefaults.bool(forKey: Keys.enforceNative))")
        
        if isFirstLaunch {
            // ПЕРВЫЙ ВХОД - идем по всем проверкам
            print("[FLOW] 📍 First launch - running all validations")
            service.performValidation { [weak self] shouldShowAlternative in
                guard let self = self else { return }
                
                DispatchQueue.main.async {
                    if shouldShowAlternative {
                        // Проверки пройдены - открываем WebView
                        print("[FLOW] ✅ Validation passed → Opening WebView (alternative mode)")
                        self.destination = .site(nil)
                    } else {
                        // Проверки не пройдены - открываем основное приложение
                        print("[FLOW] ❌ Validation failed → Opening native app")
                        self.userDefaults.set(true, forKey: Keys.enforceNative)
                        self.destination = .native
                    }
                    
                    self.considerReviewIfNeeded()
                }
            }
        } else {
            // НЕ ПЕРВЫЙ ВХОД
            if userDefaults.bool(forKey: Keys.enforceNative) {
                // При первом входе было открыто основное приложение - открываем его без проверок
                print("[FLOW] 📍 Not first launch + enforceNative → Opening native app without checks")
                destination = .native
                considerReviewIfNeeded()
            } else if service.hasShownAlternativeMode() {
                // При первом входе был открыт WebView - открываем сохраненную ссылку без проверок
                print("[FLOW] 📍 Not first launch + hasShownAlternativeMode → Opening saved URL without checks")
                // Валидация URL будет происходить в CoordinatorObserver при ошибках загрузки
                destination = .site(nil)
                considerReviewIfNeeded()
            } else {
                // Неизвестное состояние - идем по проверкам
                print("[FLOW] ⚠️ Unknown state - running validations")
                service.performValidation { [weak self] shouldShowAlternative in
                    guard let self = self else { return }
    ...
    private func resolveFlow() async {
        // Deprecated - using new validation service
    }
